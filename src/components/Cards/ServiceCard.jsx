...
const ServiceCard = ({ data, serviceType, doNotShowHoverEffect }) => {
  ...
  return (
    <Link to={(doNotShowHoverEffect || serviceType == 'current') ? "" : `/service-details/${bkid}`} className={`cursor-pointer w-full ${serviceType == "current" ? "cursor-text bg-orange-100 rounded-[15px]" : `border-b border-primary ${doNotShowHoverEffect ? "" : "hover:bg-orange-100 hover:rounded-[15px]"}`} flex flex-col gap-[12px] p-[10px]`}>
